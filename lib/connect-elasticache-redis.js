...
module.exports = function (session) {
  ...
  class RedisStore extends Store {
    constructor(options = {}) {
      ...
      this.clientMaster = options.clientMaster

      // testing replica node
      if (!options.clientReplica) {
        this.clientReplica = options.clientMaster
      } else {
        this.clientReplica = options.clientReplica
      }

      this.ttl = options.ttl || 86400*7 // 7 days in seconds.
    ...
    get(sid, cb = noop) {
      ...
      this.clientReplica.get(key, (err, data) => {
    ...
    touch(sid, sess, cb = noop) {
      ...
      let key = this.prefix + sid
      this.clientMaster.expire(key, this._getTTL(sess), (err, ret) => {
        if (err) return cb(err)
    ...
    destroy(sid, cb = noop) {
      let key = this.prefix + sid
      this.clientMaster.del(key, cb)
    }

    clear(cb = noop) {
      this._getAllKeys((err, keys) => {
        if (err) return cb(err)
        this.clientMaster.del(keys, cb)
      })
    }
    ...
    all(cb = noop) {
      ...
      this._getAllKeys((err, keys) => {
        ...
        this.clientMaster.mget(keys, (err, sessions) => {
          if (err) return cb(err)
    ...
    _scanKeys(keys = {}, cursor, pattern, count, cb = noop) {
      ...
      this.clientMaster.scan(args, (err, data) => {
        if (err) return cb(err)
